...
imu_data_path = base_path / "aob_veri_gazebo.txt"

imu_data_gazebo_txt = open(imu_data_path, 'w')

class IMUDataConverter(Node):
    ...
    def __init__(self,qos_profile):
        ...
        self.imu_pub = self.create_publisher(HtNavImuData, 'ht_nav_imu_data_topic', qos_profile=qos_profile)
        # Initialise subscribers
        self.imu_sub = self.create_subscription(Imu, 'kobra_mk5/imu_data', self.sub_cb, qos_profile=qos_profile)
...
def main(args=None):
    ...
    imu_data_converter = IMUDataConverter(custom_qos_profile)
