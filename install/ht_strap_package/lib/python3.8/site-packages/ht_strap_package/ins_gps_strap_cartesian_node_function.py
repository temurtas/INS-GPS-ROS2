...
strap_data_ins_gps_mid_txt = base_path2 / "strap_data_cartesian.txt"
strap_data_gazebo_ros_txt = open(strap_data_ins_gps_mid_txt, 'w')

ros_gazebo_kalman_out =  base_path2 / "kalman_out_cartesian.txt"
out_data_strap_kalman_ros_txt = open(ros_gazebo_kalman_out, 'w')

imu_data_path = base_path2 / "imu_data_car_gazebo.txt"
imu_data_gazebo_txt = open(imu_data_path, 'w')

gps_data_path = base_path2 / "gps_data_car_gazebo.txt"
gps_data_gazebo_txt = open(gps_data_path, 'w')

kalman_p_diag_path = base_path2 / "kalman_p_diag.txt"
kalman_p_diag_txt = open(kalman_p_diag_path, 'w')

kalman_dx_path = base_path2 / "kalman_dx.txt"
...
class INSGPSNode(Node):
    ...
    def sub_cb_imu_data(self, msg):
        ...
        self.total_vel_inc[0] = self.total_vel_inc[0] + self.imu_data.vel_diff.x
        self.total_vel_inc[1] = self.total_vel_inc[1] + self.imu_data.vel_diff.y
        self.total_vel_inc[2] = self.total_vel_inc[2] + self.imu_data.vel_diff.z

        self.total_ang_inc[0] = self.total_ang_inc[0] + self.imu_data.ang_diff.x
        self.total_ang_inc[1] = self.total_ang_inc[1] + self.imu_data.ang_diff.y
        self.total_ang_inc[2] = self.total_ang_inc[2] + self.imu_data.ang_diff.z
    ...
    def node_covariance_update(self):
        ...
        self.f_matrix = f_matrix_construct(new_pos, new_vel, new_quaternion, self.total_vel_inc)
        self.q_matrix = q_matrix_construct(new_pos, new_vel, new_quaternion, self.total_vel_inc)
