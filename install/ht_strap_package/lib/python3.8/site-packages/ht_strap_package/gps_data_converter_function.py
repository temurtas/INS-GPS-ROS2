...
gps_data_path = base_path / "gps_veri_gazebo.txt"
...
class GPSDataConverter(Node):
    ...
    def __init__(self,qos_profile):
        super().__init__('gps_data_converter')
        self.gps_data_out = HtNavGpsData()
        self.gps_data_in = NavSatFix()

        # Initialise publishers
        self.gps_pub = self.create_publisher(HtNavGpsData, 'ht_nav_gps_data_topic', qos_profile=qos_profile)
        # Initialise subscribers
        self.gps_sub = self.create_subscription(NavSatFix, 'kobra_mk5/gps_data', self.sub_cb, qos_profile=qos_profile)
